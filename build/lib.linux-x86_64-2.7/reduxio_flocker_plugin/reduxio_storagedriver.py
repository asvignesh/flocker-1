...
class ReduxioStorageDriverAPI(object):
    def __init__(self, cluster_id, rdx_ip, password):
        ...
        logging.debug(self.initiator_name)
    ...
    def create_volume_with_profile(self, dataset_id, size, profile_name):
        ...
        try:
            ...
            logging.debug("Volume created succcesffullty")
            logging.debug(blockdevice_id)
        except Exception as e:
            logging.error(
                "Cannot create volume because of exception : " + str(e))
            raise VolumeCreationFailure(e)
        volume = self.build_block_device_struct(blockdevice_id, dataset_id, size)
    ...
    def destroy_volume(self, blockdevice_id):
        """
        method to destroy a created volume
        :param blockdevice_id: WWID in the reduxio
        :return: None
        """
        volume_info = self._rdxapi.find_volume_by_wwid(wwid=blockdevice_id)[0]
        try:
    ...
    def attach_volume(self, blockdevice_id, attach_to):
        """
        attach a created volume to the host
        ...
        try:
            ...
            attached_volume = self.build_block_device_struct_a(attach_to, blockdevice_id, dataset_id, volume_size)

            logging.debug("Rescanning scsi bus for attached disk")
            _manage_session(ip_addr="10.17.180.221", port=3260)
            rescan_iscsi()
            # self._rescan_scsi()
    ...
    def build_block_device_struct_a(self, attach_to, blockdevice_id, dataset_id, volume_size):
        return BlockDeviceVolume(
            size=volume_size,
            dataset_id=dataset_id,
    ...
    def list_volumes(self):
        try:
            ...
            for vol in vol_list:
                ...
                #     blockdevice_id=unicode(vol[u'wwid']),
                #     size=vol[u'size'],
                #     attached_to=attached_to,
                #     dataset_id=dataset_id))
                volumes.append(self.build_block_device_struct_a(attached_to, unicode(vol[u'wwid']), dataset_id,
                                                                vol[u'size']))
